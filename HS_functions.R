...
#     MODIFIED:	James Foster              DATE: 2026 06 26
#
#  DESCRIPTION: Functions for Tocco C et al. (in prep).
#               
#       INPUTS: 
#               
#      OUTPUTS: 
...
MuDiff = function(id, #individual name
...
KappaDiff = function(id, #individual name
                  dta, #dataframe to use, contains columns "mu" (degrees) & "ID"
                  vars = c('cel_body', 'Shade.NoShade'), #variables of interest
                  val, #name of target condition
                  ref_val = 'sun NoShade' # name of reference condition
...
RhoDiff = function(id, #individual name
                  dta, #dataframe to use, contains columns "mu" (degrees) & "ID"
                  vars = c('cel_body', 'Shade.NoShade'), #variables of interest
                  val, #name of target condition
                  ref_val = 'sun NoShade' # name of reference condition
                    ...
                    na.rm = TRUE)
{
  switch(
    ...
    two.sided = sign(mean(x > ref, na.rm = na.rm) - 0.5) * max(c(
      mean(x > ref, na.rm = na.rm), mean(x < ref, na.rm = na.rm)
    )),
    less = mean(x < ref, na.rm = na.rm),
    greater = mean(x > ref, na.rm = na.rm),
    #for two sided, indicate direction with sign
    sign(mean(x > ref, na.rm = na.rm) - 0.5) * max(c(
      mean(x > ref, na.rm = na.rm), mean(x < ref, na.rm = na.rm)
    ))
  )
}
                 ...
                 side = 1)
{
  ...
  if(plot_rho)
  {
    arrows.circular(x = mean.circular(ca, na.rm = TRUE),
                    ...
                    ...)#passed to lines
{
  ...
  with(subset(x = dta,
              subset = ID %in% id),
       {
         lines(x = 1:length(unique(row_pastes)),
               y = mean_vector[ match(x = unique(row_pastes)[ord],#TODO check why this order
                                      table =  stim,
                                      nomatch = NA) ],
               col = gray(0,0.2),
               ...
  ...
  ...)#passed to lines
{
  ...
  with(subset(x = dta,
       ...
       {
         lines(x = 1:length(unique(row_pastes)),
               y = kappa[ match(x = unique(row_pastes)[ord],#TODO check why this order
                                      table =  stim,
                                      nomatch = NA) ],
               col = gray(0,0.2),
               ...
                       ...
                       ...)#passed to lines
{
  ...
  with(subset(x = dta,
       ...
       {
         lines(x = 1:length(unique(row_pastes)),
               y = iskappa[ match(x = unique(row_pastes)[ord],#TODO check why this order
                                      table =  stim,
                                      nomatch = NA) ],
               col = gray(0,0.2),
               ...
                  ...
                  ...)#passed to lines
{
  ...
  with(subset(x = dta,
       ...
       {
         lines(x = 1:length(unique(row_pastes)),
               y = mu[ match(x = unique(row_pastes)[c(4,1,3,2)],#c(1,2,4,3),#expected order, was previously c(1,3,2,4)
                                      table =  stim,
                                      nomatch = NA) ],
               col = gray(0,0.2),
               ...
...
AzElEst = function(sigma_phi,#circular SD and concentration
                   alpha, #observed circular CD
                   elevation, #known elevation
                   priors = c( #sd of a normal distribution
...
AzElOptim = function(alpha,
                     elevation,
                     start_par = rnorm(2),
                     ... # passed to optim
)
{
  sig_opt =  optim(par = rnorm(2),#starting values from normal(0,1)
                   f = AzElEst,
                   alpha = alpha,
                   elevation = elevation,
                   ...
  ...
  sig_opt = within(sig_opt,
                   {
                     ...
                     phi = exp(par[2])
                     #quantiles
                     lower = pi*qBeta(p = 0.025,
                                      mu = sigma/pi,
                                      phi = phi)
                     upper = pi*qBeta(p = 0.975,
                                      mu = sigma/pi,
                                      phi = phi)
                     #variance of a beta distribution
                     var = pi*((sigma/pi) * (1-sigma/pi))/
                       (phi + 1)
...
PredEleError  = 
  function(elevation, # elevation in radians
           sigma = NULL, # minimum error in radians (elevation == 0)
           kappa = NULL) # kappa on identity scale
  {
    ...
    alpha = 2 * atan(tan(sigma / 2) /
                     cos(elevation))
    return(alpha)
  }

LinesEleError = function(azel_list,
                        ...
                        ...)#additional arguments passed to lines()
{
  with(azel_list, {
    lines(x = xx,
          y = deg(PredEleError(
            elevation = elevation, sigma = sigma
          ))
          ,
          lty = 2,
          col = col,
    ...
    lines(x = xx,
          y = deg(PredEleError(
            elevation = elevation, sigma = lower
          ))
          ,
          lty = 3,
          col = col,
    ...
    lines(x = xx,
          y = deg(PredEleError(
            elevation = elevation, sigma = upper
          ))
          ,
          lty = 3,
          col = col,
...
AzElTest = function(alpha,
      ...
      res_table = data.frame(
        mu = c(alt = deg(mean(est_alpha)), 
               null = deg(plogis(null_opt$par[1])) *
                 pi),
        phi = c(alt = azel_list$phi, 
                null = exp(null_opt$par[2])),
        log_likelihood = c(alt = ll_AzEl, 
                           null = -(null_opt$value)),
        bic = -2 * c(alt = ll_AzEl, 
                     null = -(null_opt$value)) + 
          c(2, 2) * #N.B. both models really have the same d.f. 
          log(length(alpha)), 
        chi_sq = c(NA, abs(-2 * diff(c(
        alt = ll_AzEl, null = -(null_opt$value)
      ...
      Bayes_factor = c(NA, 1 / (exp(diff(
        -2 * c(alt = ll_AzEl, null = -(null_opt$value)) +
          c(2, 2) * #N.B. both models really have the same d.f.
          log(length(alpha))
      ) / 2)))[rev(rank(-2 * c(
        alt = ll_AzEl, null = -(null_opt$value)
      ) + c(2, 2) * log(length(alpha))))] #order by BIC
      )
